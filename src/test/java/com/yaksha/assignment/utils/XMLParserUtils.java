...
public class XMLParserUtils {
	...
	public static boolean checkBeanScope(String filePath, String beanId, String expectedScope) {
		try {
			File xmlFile = new File(filePath);
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
			factory.setNamespaceAware(true); // Important to handle namespaces correctly
			DocumentBuilder builder = factory.newDocumentBuilder();
			Document document = builder.parse(xmlFile);
			document.getDocumentElement().normalize();
			...
			for (int i = 0; i < beanNodes.getLength(); i++) {
				...
				if (beanNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					// If scope is missing or not the expected scope, fail the test
					if (beanId.equals(beanIdValue)) {
						...
						if (!scope.equals(expectedScope)) {
							System.out.println("Failure: 'scope' for bean '" + beanId + "' is not set to '"
									+ expectedScope + "'. Found: " + scope);
							return false;
						}
